  ...
  return (
    <div className="px-5 md:px-10 py-8">
      <div className="flex items-center justify-between gap-3">
        <Link href="/now-showing" className="inline-flex items-center gap-1.5 text-gold text-sm font-semibold">
          <ArrowLeft size={15} /> Back to Now showing
        </Link>
        <Link
          href={`/now-showing/${movie.id}?kind=${otherKind}`}
          className="flex items-center gap-1.5 text-xs font-semibold text-textDim bg-surface shadow-card rounded-full px-3.5 py-2 hover:text-gold transition"
        >
          <RefreshCw size={13} />
      ...
      <div className="mt-6 flex flex-col md:flex-row gap-6 items-start">
        <div className="w-40 h-60 flex-none rounded-2xl overflow-hidden shadow-card relative bg-surface2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-gold bg-tintBlue rounded-full px-3 py-1 mt-3 capitalize">
            {kind} data
          </div>

          {kind === 'tracked' ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-5">
              <StatCard label="Lifetime gross" value={movie.lifetime_gross} highlight />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-5">
              <StatCard label="Advance gross" value={movie.advance_gross} highlight />
      ...
      {review && (
        <Card className="mt-8 p-5 max-w-xl">
          ...
          <Link href={`/reviews/${review.id}`} className="text-gold text-xs font-semibold">
            Read full review →
          </Link>
        </Card>
      )}
      ...
      {(movie.total_worldwide || movie.total_india_gross) && (
        <div className="mt-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard label="Worldwide" value={movie.total_worldwide} highlight />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
            ...
            <StatCard label="Box Office Verdict" value={movie.box_office_verdict} />
          </div>
        </div>
      )}
        ...
        <div className="mt-10">
          ...
          <Card className="overflow-x-auto">
            <table className="w-full text-sm border-collapse min-w-[560px]">
              ...
              <tbody>
                {dailyRows.map((d: any) => (
                  <tr key={d.id} className="border-b border-[#F2F4F7] last:border-0">
